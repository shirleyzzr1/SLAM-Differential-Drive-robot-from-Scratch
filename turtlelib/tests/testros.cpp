...
TEST_CASE("test dirve foward kinematics","dirve foward"){
    ...
    REQUIRE(diff.wheel_vel().x==Approx(6.66).epsilon(0.01));
    REQUIRE(diff.wheel_vel().y==Approx(6.66).epsilon(0.01));

}
TEST_CASE("test drive rotational kinematics","dirve rotational"){
    DiffDrive diff;
    diff.FK_calculate({5,-5});
    REQUIRE(diff.body_pos().rotation()==Approx(-2.06).epsilon(0.01));
    REQUIRE(diff.body_pos().translation().x==Approx(0));
    REQUIRE(diff.body_pos().translation().y==Approx(0));

    Twist2D tw = {2.84,0,0};
    diff.IK_calculate(tw);
    REQUIRE(diff.wheel_vel().x==Approx(-6.88).epsilon(0.01));
    REQUIRE(diff.wheel_vel().y==Approx(6.88).epsilon(0.01));

}
TEST_CASE("test drive circle kinematics","dirve circle"){
    DiffDrive diff;
    diff.FK_calculate({29.757,30.848});
    REQUIRE(diff.body_pos().rotation()==Approx(0.225).epsilon(0.01));
    REQUIRE(diff.body_pos().translation().x==Approx(0.99).epsilon(0.01));
    REQUIRE(diff.body_pos().translation().y==Approx(0.112).epsilon(0.01));
...
TEST_CASE("test impossible twist","dirve impossible"){
    ...
    Twist2D tw = {1,1,1};
    diff.IK_calculate(tw);
    // REQUIRE_CATCH(diff.IK_calculate(tw));
}
