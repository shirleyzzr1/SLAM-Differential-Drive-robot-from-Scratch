...
public:
    ...
    ros::Publisher red_joint_pub;
    ros::Publisher blue_joint_pub;
...
void Message_handle::sensor_callback(const nuturtlebot_msgs::SensorData& msg){
    ...
    sensor_msgs::JointState joint_state_blue;
    joint_state_blue.name.push_back("blue/wheel_left_joint");
    joint_state_blue.name.push_back("blue/wheel_right_joint");
    //convert the encoder tick to wheel_position
    joint_state_blue.position.push_back(this->left_encoder* 2 * turtlelib::PI/encoder_ticks_to_rad);
    joint_state_blue.position.push_back(this->right_encoder* 2 * turtlelib::PI/encoder_ticks_to_rad);

    joint_state_blue.velocity.push_back(this->diffdrive.wheel_vel().x);
    joint_state_blue.velocity.push_back(this->diffdrive.wheel_vel().y);   

    joint_state_blue.header.stamp = ros::Time::now();
    this->blue_joint_pub.publish(joint_state_blue);

    sensor_msgs::JointState joint_state_red;
    joint_state_red.name.push_back("red/wheel_left_joint");
    joint_state_red.name.push_back("red/wheel_right_joint");
    //convert the encoder tick to wheel_position
    joint_state_red.position.push_back(this->left_encoder* 2 * turtlelib::PI/encoder_ticks_to_rad);
    joint_state_red.position.push_back(this->right_encoder* 2 * turtlelib::PI/encoder_ticks_to_rad);

    joint_state_red.velocity.push_back(this->diffdrive.wheel_vel().x);
    joint_state_red.velocity.push_back(this->diffdrive.wheel_vel().y);   

    joint_state_red.header.stamp = ros::Time::now();
    this->red_joint_pub.publish(joint_state_red);
...
int main(int argc, char ** argv){
    ...
    msgh.blue_joint_pub = n.advertise<sensor_msgs::JointState>("blue/joint_states",1000);
    msgh.red_joint_pub = n.advertise<sensor_msgs::JointState>("red/joint_states",1000);
